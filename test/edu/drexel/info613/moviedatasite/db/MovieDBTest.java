...
public class MovieDBTest extends TestCase {
    ...
    public void testGetMoviesByActor() {
        ...
        if (result.size() > 0) {
            String[] movieTitle = new String[result.size()];
            String[] movieGenre = new String[result.size()];

            for (int i = 0; i < result.size(); i++) {
                movieTitle[i] = result.get(i)[0];
                movieGenre[i] = result.get(i)[1];
                assertEquals(movie.getTitle(), movieTitle[i]);
                //linked list adds element to the tail, so the first element of array
                //matches up the last element of linked list 
                assertEquals(movie.getGenres().get(result.size() - i).getName(), movieGenre[i]);
            }
        }
    ...
    public void testGetMoviesByDirector() {
        ...
        if (result.size() > 0) {
            ...
            for (int i = 0; i < result.size(); i++) {
                ...
                assertEquals(movie.getGenres().get(result.size() - i).getName(), result.get(i)[1]);
            }
        }
